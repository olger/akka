...
object Sink {
   ...
   */
  def file(f: File): javadsl.Sink[ByteString, Future[Long]] = file(f, append = false)
   ...
   * set it for a given Source by using [[ActorAttributes]].
   */
  def file(f: File, append: Boolean): javadsl.Sink[ByteString, Future[Long]] =
    new Sink(scaladsl.Sink.file(f, append))
